...
def getDitalData():
    train_data = extract_data("data/train.pkl")
    test_data = extract_data("data/test.pkl")
    label = extract_data("data/labels.pkl")
    print train_data.shape, label.shape, test_data.shape
    return train_data, label, test_data

def main():
    train_data, train_labels, test_data = getDitalData()
  ...
  # Create a local session to run the training.
    ...
    output = numpy.argmax(preLabels, 1)
    pred_file = open("data/res.csv","wb+")
    pred_writer = csv.writer(pred_file)
    ids = range(1,len(output)+1)
    pred_writer.writerow(["ImageId","Label"])
    pred_writer.writerows(zip(ids,output))
    pred_file.close()
    sess.close()
